...
Companion to `/Users/Suhas.KS/gcp.md` ("The Consolidated Cloud Mastery Curriculum")
...
**This file is a complement to `/Users/Suhas.KS/gcp.md`, not a second curriculum. Read both. Whenever a gcp.md module is taught, also teach every companion concept bound to it (Section 2) in the same session, as one story. Similar, related, and overlapping concepts are stitched together and taught in parallel — never in separate sessions, never twice.**

Why: the primer teaches *what* scalable systems are made of, in vendor-neutral words (load balancer, cache, queue, shard). gcp.md teaches *which real cloud resources* those words become, and the exam-grade judgment around them. Industry builds high-scale systems out of cloud resources, so every primer concept must arrive with its GCP resource attached — and every GCP resource should arrive with the primer's trade-off reasoning attached.

### 0.2 Stitching rules

1. **One concept, one teaching.** Where both files cover the same idea (L4 vs L7 load balancing, CAP, least privilege, queues, Big-O, TLS, replication), teach it once, in the gcp.md module that owns it. This file adds the *system-design layer*: trade-offs and disadvantages, numbers, interview framing, and the GCP resource. Later sessions refer back; they don't re-teach.
...
3. **AWS and Azure names come from gcp.md Part VIII — don't teach three times.** The primer's own examples are AWS-flavoured (S3, RDS, ELB, CloudFront, ElastiCache, SQS, Route 53, DynamoDB, Redshift, CloudWatch); every concept below translates them to GCP, and Part VIII gives the Azure side.
4. **Order and pacing belong to gcp.md** (Phase plan, Part X, "skip ahead" rights). **SD content and problems belong to this file.** On a conflict, gcp.md wins on order, Lab Reality, and exam time-sensitivity; this file wins on system-design content.
...
8. **User can override anything:** skip a concept already known, jump to a specific problem, or go hands-on — same rights as gcp.md Part X.
9. **Read economically.** This file is large by design. Each session read §0 and §2, then only the blocks bound to today's gcp.md module (search by ID: `SD-10`, `SX-05`, `P02`…). Do not reload the whole file, and do not copy its contents into other files.

### 0.3 How one stitched session runs

1. **Anchor** — announce the gcp.md module and list the companion concepts bound to it (Section 2).
2. **Concept** — teach the shared idea once (gcp.md depth) and layer the primer's trade-offs and disadvantages on top.
...
7. **Close** — tick the boxes in both files' sense (gcp.md module + companion concepts).
...
- `A1…A11, B1…B5, C1…C7, D1…D4` — gcp.md module IDs. `Part V` = gcp.md's GCP service map.
...
Each gcp.md module on the left is taught **with** the companion concepts on the right, in the same session (Section 0.2 rule 1). "Checkpoint" is the problem to run once that module and its stitched concepts are done.

| gcp.md module | Companion concepts taught in the same session | Checkpoint |
|---|---|---|
...
|---|---|---|
| L4 vs L7 load balancing, algorithms | gcp.md A5 | SD-10: SSL termination, session persistence, LB failover, disadvantages |
| Reverse proxy vs forward proxy | gcp.md A5 / C3 | SD-11: benefits list, LB-vs-RP decision rule |
| DNS record types | gcp.md A5 | SD-08: TTL staleness, weighted/latency/geo routing, DNS-as-SPOF |
| TCP vs UDP | gcp.md A5 | SD-30/31: when-to-use rules, connection pooling |
| Message queues / event-driven | gcp.md A7 | SD-28: task queues, back pressure, delivery semantics |
| REST / gRPC | gcp.md A7 | SD-32/33/34: RPC-vs-REST decision, HATEOAS |
| ACID, NoSQL families, CAP | gcp.md A8 / A9 | SD-13…SD-25: scaling techniques, SQL-vs-NoSQL decision lists |
| Replication, sharding, consensus | gcp.md A9 | SD-06, SD-14, SD-15, SD-17, SD-38 |
| Availability vs durability, nines | gcp.md A9 / C7 | SD-07 tables and series/parallel formulas |
| HA, horizontal vs vertical scaling, DR | gcp.md B3 | SD-06, SD-10, P08 walk-through |
| Least privilege, encryption | gcp.md A10 / B5 | SD-35 checklist |
| Big-O, hash lookup, indexes | gcp.md A2 / A4 | SD-19 index trade-offs, SX-11 |
| Observability, SRE | gcp.md C6 / C7 | P08 monitoring list, back pressure |
| Autoscaling (HPA, MIG) | gcp.md C2 / Part V | P08 "Users++++" |
...
#### SD-00 · The interview method + back-of-the-envelope — stitch: A2, A11, B3
- [ ] done
...
#### SD-01 · Scalability: start here — stitch: A6, B2, C1, Part V Compute
- [ ] done
...
#### SD-02 · Performance vs scalability — stitch: A2, B3
- [ ] done
...
- **GCP lens:** performance → per-request latency in Cloud Trace (gcp.md C6); scalability → behaviour under load in Cloud Monitoring while autoscaling (MIG autoscaler, Cloud Run instance scaling, GKE HPA).
...
#### SD-03 · Latency vs throughput — stitch: A2, C6, C7
- [ ] done
...
#### SD-04 · CAP theorem — stitch: A8, A9
- [ ] done
...
#### SD-05 · Consistency patterns — stitch: A9
- [ ] done
...
#### SD-06 · Availability patterns: fail-over and replication — stitch: A9, B3, C7
- [ ] done
...
- **GCP lens:** regional MIG spread across zones with health-check **autohealing** replaces heartbeat + IP takeover; global external Application LB shifts traffic away from unhealthy backends/regions automatically; Cloud SQL **HA** = regional primary + synchronous standby in another zone, automatic fail-over keeping the same IP; cross-region replicas for DR; Cloud DNS failover/weighted policies for DNS-level steering. VPC does not support L2 tricks (VRRP/gratuitous ARP) `verify`, so "take over the IP" becomes LB health checks or API-driven IP reassignment. Map gcp.md B3 DR patterns: backup/restore → cold standby; pilot light/warm standby → active-passive; multi-site → active-active.
...
#### SD-07 · Availability in numbers — stitch: A2, C7
- [ ] done
...
#### SD-08 · Domain name system — stitch: A5, Part V Networking
- [ ] done
...
#### SD-09 · Content delivery network — stitch: A5, B4, Part V Networking
- [ ] done
...
#### SD-10 · Load balancer (+ horizontal scaling) — stitch: A5, C2, C3, B3, Part V Networking
- [ ] done
...
- **GCP lens:** **Cloud Load Balancing** is fully managed and distributed — you don't fail it over, which dissolves most of the "LB is a SPOF" worry. **Application LBs (L7):** global external, regional external, internal; URL maps do the primer's "video vs billing" split (host/path rules → backend services); health checks; backends = MIGs, NEGs (zonal, serverless, internet). **Network LBs (L4):** proxy (TCP/SSL) and passthrough (TCP/UDP, preserves client IP). Session affinity: client IP, generated cookie, header, HTTP cookie. Managed certificates = SSL termination. Attach Cloud Armor for WAF/DDoS. Autoscaling: MIG autoscaler on CPU / LB utilization / custom metric / schedule (gcp.md Part V). AWS ELB/ALB/NLB ↔ Part VIII.
...
#### SD-11 · Reverse proxy (web server) — stitch: A5, C3
- [ ] done
...
- **GCP lens:** the global external ALB *is* a managed reverse proxy (TLS termination, host/path routing, Cloud CDN caching, Cloud Armor, IAP for identity-aware access). Cloud Run's built-in HTTPS front end is one too. Self-managed: NGINX on GCE/GKE (gcp.md C3), NGINX Ingress. API-facing proxying: API Gateway or Apigee (quotas, keys, transformations). East-west: Cloud Service Mesh (Envoy). Serve static content from Cloud Storage + CDN, not from the app.
...
#### SD-12 · Application layer: microservices & service discovery — stitch: A7, C2, Part V Compute
- [ ] done
...
#### SD-13 · Relational databases & ACID — stitch: A8, Part V Storage/DB
- [ ] done
...
#### SD-14 · Master-slave replication — stitch: A9, Part V Storage/DB
- [ ] done
...
#### SD-15 · Master-master replication — stitch: A9
- [ ] done
...
#### SD-16 · Federation (functional partitioning) — stitch: A8, A7
- [ ] done
...
#### SD-17 · Sharding — stitch: A9, Part V Storage/DB
- [ ] done
...
#### SD-18 · Denormalization — stitch: A8
- [ ] done
...
#### SD-19 · SQL tuning — stitch: A4, A6, C6, Part V Ops
- [ ] done
...
#### SD-20 · NoSQL & BASE — stitch: A8, A9, Part V Storage/DB
- [ ] done
...
#### SD-21 · Key-value store — stitch: A4, A8
- [ ] done
...
#### SD-22 · Document store — stitch: A8
- [ ] done
...
#### SD-23 · Wide-column store — stitch: A8, A9
- [ ] done
...
#### SD-24 · Graph database — stitch: A4, A8
- [ ] done
...
#### SD-25 · SQL or NoSQL — stitch: A8, gcp.md Cloud Database Engineer domain
- [ ] done
...
#### SD-26 · Cache: where and what — stitch: A5, B4, C3, Part V Storage/DB
- [ ] done
...
#### SD-27 · When to update the cache — stitch: A3, A9
- [ ] done
...
#### SD-28 · Asynchronism: message queues, task queues, back pressure — stitch: A7, C7, Part V Data/Analytics
- [ ] done
...
#### SD-30 · TCP — stitch: A5, A6, Part V Networking
- [ ] done
...
#### SD-32 · Remote procedure call (RPC) — stitch: A7, A3
- [ ] done
...
- **GCP lens:** **gRPC = Protobuf over HTTP/2**, native on Cloud Run and GKE; Cloud Endpoints/API Gateway support gRPC and **transcoding** (one `.proto` → REST + gRPC); Cloud Service Mesh adds retries/mTLS; Pub/Sub schemas accept Avro and Protobuf. Google's own `google-cloud-*` client libraries are the "native SDK" option (gcp.md A3).
...
#### SD-33 · REST (including HATEOAS) — stitch: A7, A3
- [ ] done
...
#### SD-35 · Security basics — stitch: A10, B5, Part V Security
- [ ] done
...
- **GCP lens:** cloud bills are in binary-vs-decimal units (gcp.md A1) — Cloud Storage documents GB as 2^30 bytes in its pricing terms (`verify`); IPv4 has 2^32 addresses; the primer's `INT` "up to 4 billion" is 2^32; Base62^7 ≈ 3.5×10^12 (SX-02).
...
#### SD-37 · Latency numbers every programmer should know — stitch: A1, A6, C6
- [ ] done
...
#### SD-38 · The three "under development" topics: consistent hashing, MapReduce, scatter-gather — stitch: A4, A9, Part V Data/Analytics
- [ ] done
...
#### SD-39 · Real-world architecture papers — stitch: A9, Part V (all)
- [ ] done
...
```mermaid
graph TD
  A1[gcp A1 binary and units] --> SD36[SD-36 powers of two]
  SD36 --> SD37[SD-37 latency numbers]
  SD37 --> SD00[SD-00 method and BoE]
  A5[gcp A5 networking] --> SD08[SD-08 DNS]
  A5 --> SD29[SD-29 HTTP]
  A5 --> SD30[SD-30 TCP]
  SD30 --> SD31[SD-31 UDP]
  ...
  A9[gcp A9 distributed systems] --> SD04[SD-04 CAP]
  SD04 --> SD05[SD-05 consistency]
  SD05 --> SD06[SD-06 availability patterns]
  SD10 --> SD06
  SD06 --> SD07[SD-07 nines]
  A8[gcp A8 databases] --> SD13[SD-13 RDBMS and ACID]
...
| Tier | You are ready for it when… | Concepts complete | gcp.md modules complete |
|---|---|---|---|
| **T0 Foundations** | — | SD-36, SD-37, SD-00 | A1, A2 |
| **T1 Single box + edge** | you can explain what happens between typing a URL and the first byte | SD-01, SD-02, SD-03, SD-08, SD-29, SD-30, SD-31 | A5, A6 (+A3 for code) |
| **T2 Scale-out basics** | you can add a second server without breaking sessions | SD-09, SD-10, SD-11, SD-12, SD-33, SD-34, SD-35 | A7, C3, Part V Networking at Lens-1 |
| **T3 Data at scale** | you can state a consistency choice per dataset | SD-04 … SD-07, SD-13 … SD-25 | A8, A9, Part V Storage/DB at Lens-2 |
| **T4 Speed & decoupling** | you can defend a cache strategy and a queue | SD-26, SD-27, SD-28, SD-32, SD-38, SX-01 … SX-13 (as needed) | C2, C6, Part V Data/Analytics at Lens-1 |
| **T5 Open-ended design** | you can run the four-step method unprompted | all SD-nn | Phase 3 done; Phase 4 begun |
...
| ID | Problem | Tier | Must know | New here | gcp.md gate | Builds on | Unlocks |
|---|---|---|---|---|---|---|---|
| **P08** | Scale to millions of users (AWS in primer → GCP here) | T1 outline; T5 capstone | first pass: SD-00 … SD-03; second pass: everything through T4 | SX-12 (the loop) — each "Users+" step introduces one concept | A5, A6, B3 (first pass); C2, C6, Part V Compute/Networking (second) | — | every problem below; also the AWS/Azure re-runs (Parts VI/VII) |
| **P01** | Pastebin.com / Bit.ly | T2 | SD-00, SD-36/37, SD-11, SD-12, SD-13, SD-19, SD-25, SD-32/33 | SX-01, SX-02, SX-03, SX-04 | A3, A4, A5, A8 | P08 (outline) | P06, P07, P02, Q08, Q17 |
| **P06** | Key-value cache for search-engine queries | T4 | SD-21, SD-26, SD-27, SD-05, SD-37 | SX-11, SD-38a (consistent hashing) | A4, A9 | O02, P01; P03 (soft — only its "Reverse Index Service" and "Document Service" boxes, which P06 caches) | Q05, Q06, Q22 |
| **P07** | Amazon sales rank by category | T4 | SD-13, SD-19, SD-25, SD-26, SD-17, SX-01, SX-03 | SD-38b, SX-08 (multi-step MapReduce) | A2, Part V Data/Analytics (Dataflow/BigQuery) Lens-1 | P01 | P04, Q16, Q18 |
| **P04** | Mint.com | T4 | SD-13, SD-19, SD-26/27, SD-28, SD-35, SD-25, SX-01, SX-03 | SX-09, SX-13 | A7, A8, A10 | P01, P07 | Q07 (pipelines) |
| **P03** | Web crawler | T4 | SD-08, SD-21, SD-28, SD-17, SD-30/31, SD-26, SD-05 | SX-06, SX-07 | A4 (graphs, cycles), A5 | P01, P07 (MapReduce) | P06, Q02, Q03 |
| **P02** | Twitter timeline & search | T4 | SD-02, SD-03, SD-12, SD-13 … SD-19, SD-20/21/23, SD-26/27, SD-28, SD-05 | SX-05, SX-06, SD-38c | A8, A9, Part V Storage/DB Lens-2 | P01, P06, P08 | Q01, Q09–Q14, Q16 |
| **P05** | Social-network data structures (shortest path) | T4 | SD-17, SD-24, SD-26, SD-32, SD-37, A4 (graphs/BFS) | SX-10 | A4, A9 | P06, P02 | Q14 |
| **O01** | Hash map | T0 | A3, A4 | — (SD-21 in miniature) | A3, A4 | — | O02, P06, Q05 |
| **O02** | LRU cache | T0 | O01, A4 (doubly linked list) | SX-11 | A3, A4 | O01 | P06, Q06 |
| **O03** | Call center | T0 | A3 (abstract classes, enums), A4 (queue) | — | A3, A4 | — | (queue/escalation patterns: SD-28) |
| **O04** | Deck of cards (+ blackjack) | T0 | A3 (inheritance, properties) | — | A3 | — | Q20 |
| **O05** | Parking lot | T0 | A3 (composition, polymorphism) | — | A3 | — | (concurrency extension: SD-13 transactions) |
| **O06** | Online chat (users, friends, private/group chat) | T2 | A3, SD-12, SD-13 | — | A3, A7 | O03 (state/roles) | Q09, Q13 |
| **O07** | Circular array (primer placeholder, no solution) | T0 | A3, A4 (arrays, modulo) | ring buffer | A3, A4 | O01 | (bounded buffers: SD-28 back pressure) |
| **Q01** | File sync like Dropbox | T5 | SX-01, SD-13/25, SD-28, SD-05, SD-09, SD-35 | chunking + content-hash dedup, delta sync | A8, A10 | P01, P02 | — |
| **Q02** | Search engine like Google | T5 | SX-06, SX-07, SD-38b/c, SD-17, SD-26 | ranking (PageRank was out of P03's scope) | D4 (embeddings) at Lens-1 | P03, P06 | — |
| **Q03** | Scalable web crawler like Google | T5 | P03 + SD-17 | politeness per host, frontier partitioning, freshness scheduler | Part V Data/Analytics | P03 | Q02 |
| **Q04** | Google Docs | T5 | SD-04, SD-05, SD-28, SD-29, A9 | operational transform / differential sync (CRDTs: my addition) | A9 | O06, P02 | — |
| **Q05** | Key-value store like Redis | T5 | SD-21, SD-17, SD-38a, SD-06, SD-14/15, SD-04, SD-05 | Dynamo techniques (quorum, vector clocks, gossip — from the linked paper) | A9 | O01, O02, P06 | Q19 |
| **Q06** | Cache system like Memcached | T4 | SD-26, SD-27, SX-11, SD-38a | slab allocation, sharded client library | A4 | O02, P06 | — |
| **Q07** | Recommendation system like Amazon's | T5 | SD-38b, SD-21/23, SD-28, SX-08 | collaborative filtering; offline batch + online serving | D1, D3 | P07 | — |
| **Q08** | TinyURL like Bitly | T2 | = P01 | — | as P01 | P01 | — |
| **Q09** | Chat app like WhatsApp | T5 | SD-28, SD-21/23, SD-05, SD-12, SD-31 | persistent connections, store-and-forward, delivery receipts, presence | A7 | O06, P02 | Q13 |
| **Q10** | Picture sharing like Instagram | T5 | SX-01, SD-09, SX-05, SD-17, SD-14, SD-26 | image pipeline, feed + media at scale | Part V Storage | P02, P01 | — |
| **Q11** | Facebook news feed | T5 | SX-05, SD-26, SD-28 | activity streams, ranking | A8 | P02 | Q12 |
| **Q12** | Facebook timeline | T5 | SD-18, SD-17, SD-20 | denormalized per-user timelines | A8 | P02, Q11 | — |
| **Q13** | Facebook chat | T5 | SD-28, SD-21, SD-12 | presence, ordering, large fan-in of connections | A7 | O06, Q09 | — |
| **Q14** | Graph search like Facebook | T5 | SX-10, SX-06, SD-24, SD-17 | indexing + ranking + natural-language front end | D4 | P05, P02 | — |
| **Q15** | CDN like Cloudflare | T5 | SD-08, SD-09, SD-10, SD-26/27 | anycast routing, tiered caching, cache invalidation | A5 | P08 | — |
| **Q16** | Trending topics like Twitter's | T5 | SD-28, SD-38b, SX-08, SD-03 | streaming windows, approximate counting (my addition) | Part V Data/Analytics (Dataflow) | P07, P02 | Q18 |
| **Q17** | Random-ID generation | T3 | SX-02, SD-17, A1 (bit layout) | Snowflake layout: timestamp + machine + sequence | A1 | P01 | — |
| **Q18** | Top-k requests in a time interval | T5 | SD-38b, SD-28 | heavy hitters / sketches (from the linked papers) | Part V Data/Analytics | P07, Q16 | — |
| **Q19** | Serve data from multiple data centers | T5 | SD-04, SD-05, SD-06, SD-07, SD-15, SD-17 | multi-region consistency and fail-over | A9, B3 | P08, Q05 | — |
| **Q20** | Online multiplayer card game | T5 | SD-30/31, SD-28, SD-05, O04 | authoritative server, turn state, latency hiding | A5 | O04, O06 | — |
| **Q21** | Garbage-collection system | T0 | A4 (graph reachability), A6 (memory) | mark-and-sweep, reference counting | A4, A6 | O01 | — |
| **Q22** | API rate limiter | T4 | SD-26, SD-28, SD-29, SD-10/11, SD-17 | token bucket, leaky bucket, fixed/sliding window | A7 | P06, P08 | — |
| **Q23** | Stock exchange (NASDAQ/Binance-style) | T5 | SD-03, SD-06, SD-13, SD-28, SD-05, SD-37, A6 | order book, matching engine, sequencer, event sourcing | A6, A9 | P08 | — |
...
- **GCP-era note:** the bill is dominated by storage and egress — 2 PB/month of new pages. Price it in the Pricing Calculator (gcp.md B4); that number is *why* storage classes, compression, and dedup exist. Crawl politely: per-host rate limits and `robots.txt` are legal and ethical, not just a freshness knob.
...
#### O01–O07 · Object-oriented design problems (taught with gcp.md A3/A4; each has a "cloud extension" that connects it to the system-design problems)
...
| Q14 | Graph search like Facebook | three FB "Under the hood" notes (infrastructure, indexing/ranking, natural-language interface) | index + rank + NL front end over a social graph | Spanner Graph/Bigtable + Elasticsearch (+ an LLM front end, gcp.md D4) |
...
### 4.5 The ladder — recommended problem order, stitched to gcp.md's phases

The order follows the prerequisite table (§4.3); each rung fires only when its gate is green. gcp.md's phase plan sets the calendar.

| gcp.md phase | Stitched concepts (taught with the module) | Problems that become available |
|---|---|---|
...
Every solution uses the same generic boxes. This is the lookup table for reading a primer diagram as a GCP diagram. AWS/Azure columns only where gcp.md Part VIII has the row.
...
### 5.2 Terraform exercises (gcp.md C5: "we lean hard on `terraform plan`")
...
Suggested pairing per phase: Phase 0 → Dynamo + Bigtable (A8/A9); Phase 2 → Dapper + Chubby (C6/C7); Phase 4 → Spanner + MapReduce + GFS (Part V Storage/Data); read Kafka alongside SD-28.
...
- Sister repo **Interactive Coding Challenges** (`donnemartin/interactive-coding-challenges`) with its own `Coding.apkg` — the coding-interview complement to gcp.md A4.
...
### 7.3 Maintenance
When gcp.md changes (new modules, reordered phases, revised exam guides), update **§2 stitch table** and **§4.5 ladder** first; the concept modules and problem cards rarely need edits. If a GCP product is renamed or retired, fix the **GCP lens**/**bill of materials** lines and leave the primer content alone.
