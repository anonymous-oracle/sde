...
**Status.** Standalone draft, self-contained by design. Nothing here has been written into
`gcp-curriculum.md`, which remains the **syllabus of record** and is untouched.
...
PART 2   The finding — what "from scratch" actually requires, and the gap
PART 3   The floor — theory spine (Block T + Part M), carried in full
...
## 0.1 Relationship to the curriculum

Part 1 (the catalog) and Part 3 (the theory spine) are **carried from `gcp-curriculum.md`** —
Appendix M lines 7749–8094, Block T lines 633–888, Part M lines 1316–1508, course spine lines
591–628, prerequisite map lines 549–588, Appendix G lines 8096–8119. They are reproduced here
so this file stands alone. **The curriculum is still the source of record; if the two ever
diverge, the curriculum wins.**

Parts 2, 4, 5, 6, 7, 8 and 9 are new and exist only here.
...
- **has the floor, or can skip-test it.** Part 3 is the floor. Part 4 assumes it. If a gate in
  Part 3 is not on the ledger, close it before opening the matching ceiling in Part 4.
...
The curriculum teaches Appendix M's families as **literacy + evidence packs**. That is enough
to *architect* a case study, defend a design in an interview, and ship the four Northstar
implementations (ranker, ETA, fraud-on-tokens, RAG). It is **not** enough to *build one from
scratch unaided*, for three reasons that fall straight out of the file:

1. **The ceilings are named but empty.** `T-RL`, `T-CAUSAL`, `T-CV`, `T-SIGNAL`, `T-DL`,
   `T-NLP` appear only as one-line scope pointers (Part 12 map lines 491–494; 9c.0 defer rows
   6116–6124) with **no assigned text and no course**. Grep counts in the curriculum: T-RL 4,
   T-CAUSAL 8, T-CV 8, T-SIGNAL 10, T-DL 9, T-NLP 3 — every occurrence a pointer, never a shelf.
2. **The two largest families have no theory owner at all.** Recommend/feed (65) +
   Search/rank/ads (36) = **101 of 309**. There is no `T-REC`, no `T-IR`, no `T-GRAPH` anywhere
   in the file (grep: 0 hits each). Manning's *Introduction to Information Retrieval* is the
   only retrieval text, entered at 9c.2 for IR **literacy** only.
3. **"Other" (101) is index-only** — pricing, routing, entity resolution, anomaly detection,
   dimension modelling have no owner and no reading. That is a third of the catalog.
...
Populating these ceilings **contradicts the curriculum's own explicit out-of-syllabus lines**:
transformer-from-scratch, full CV, full RL, full GNN, pure DSP/Kaldi, and the five GenAI
portfolio tracks are all deliberately excluded there. Those exclusions are sensible for a
GCP-certification track with a fixed time budget. They are *not* compatible with "build it
from scratch unaided."

**This file does not silently reverse them.** Part 4 is the **continuation** you open *after*
the main track, per family, when the goal changes from "architect and defend" to "build
unaided." Each ceiling in Part 4 states its own stopping point so the scope stays finite even
once opened. Which families to open is a decision, not a default.

## 2.4 What is still out of scope even with Part 4 open
...
frontier-scale LLM · re-implementing the 309 blog posts individually · a second GCP product
course. Part 4 **stocks the shelf**; it does not add 309 lessons.

---

# PART 3 — The floor: the theory spine (carried from the curriculum)

## 3.0 Placement in the course spine
...
## 3.2 Theory prerequisites map (main owner → required T.* tier)
...
## 3.4 Part M — quantitative prerequisites (owns specific derives)
...
## 3.5 Overlap bans carried forward (one home per artifact)
...
## 4.0 Bridge — which spine family feeds which ceiling
...
**Covers.** Recommend / personalize / feed (65) — the largest family, and by far the largest
single gap in the curriculum.
...
> This is the ceiling most directly in tension with the curriculum's out-of-syllabus line
> (§2.3). Opening it is a real time commitment — budget a full term, not a weekend.
...
dynamic pricing inside "Other" (101). The curriculum gives 9c.2 bandit **literacy**; the
derivation ceiling is unstocked.
...
cross-cutting and has no `T-*` label in the curriculum because Parts 3, 8 and 10 own pieces of
it — but a learner building alone needs it named in one place.
...
> **Compatibility note.** The curriculum's Appendix G (G0–G20) indexes Go modules against GCP
> owners. This track is the same language, re-cut against the **309 build** instead. Where they
> overlap, do the artifact once and claim it in both places.
...
**The rule for this curriculum.** Every numerical function in Part 6 ships with:
...
best match for this curriculum's testing-first stance; work it alongside GO-1…GO-7.
...
with the curriculum's out-of-syllabus line (§2.3). It is also the one that permanently ends
treating attention as magic. Budget accordingly — and if you skip it, skip it *deliberately*
and use the ONNX path for family 5 instead.
...
**Carried, not authored here.** Part 1 (the 309 index) and Part 3 (the theory spine) come from
`gcp-curriculum.md` at the line ranges given in §0.1. The curriculum is the source of record.

**Authored here.** Parts 0, 2, 4, 5, 6, 7, 8, 9 — the ceiling curation, the Go track, the build
ladder, the Python boundary, and the bibliography.
...
- The scope conflict in §2.3 is **flagged, not resolved**. Opening T-DL, T-CV, T-RL and T-SIGNAL
  contradicts the curriculum's stated out-of-syllabus lines. That decision has not been made.
- Part 6's acceptance tests are specified but not yet executed — no code in this repository
  implements them.
