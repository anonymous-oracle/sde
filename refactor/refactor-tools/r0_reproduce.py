...
CHECKS = [
 ...
 ("C-42", "77 primer boxes all unticked", f"grep -c '^\\s*- \\[ \\]' {P}; grep -o '\\[ \\]' {P} | wc -l; grep -c '\\[x\\]' {P}", DF, "76 line-start boxes + 2 inline '[ ]' = 78 box tokens (prompt: 77); 0 ticked. Same conflict."),
 ...
 ("C-58", "DP has one box for 63 items", f"grep -c '^\\s*- \\[ \\]' {D}; grep -o '\\[ \\]' {D} | wc -l", R, "0 line-start boxes, 1 inline box token."),
