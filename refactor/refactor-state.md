...
**Resume point:** Finished (2026-09-24). The learner closed the remaining revamp work: fix the conflicts the revamp named, keep every part aligned with no content lost, add the academic depth (D17), then put the course files in one folder. All of that is done. The finished course is in the repo's `course/` folder (the six parts and the regenerated progress ledger), written by `python3 refactor-tools/r2b_build.py .` from the same build as `work/`. `python3 refactor-tools/verify.py . --stage R5` → **PASS**, 54 of 54 GATE rows (`verification-report-R5.md`): every conflict C-01…C-75 and C-NEW-01…09 is PASS, PASS by substance or lineage, or closed by D2; none is deferred. After a fresh checkout run `chmod a-w inputs-original/*`.
