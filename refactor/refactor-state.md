...
**Resume point:** Finished (2026-09-24). The learner closed the remaining revamp work: fix the conflicts the revamp named, keep every part aligned with no content lost, add the academic depth (D17), then put the course files in one folder. All of that is done. The finished course is in the repo's `course/` folder (the six parts and the regenerated progress ledger), written by `python3 refactor-tools/r2b_build.py .` from the same build as `work/`. `python3 refactor-tools/verify.py . --stage R5` → **PASS**, 58 of 58 GATE rows (`verification-report-R5.md`): every conflict C-01…C-75 and C-NEW-01…09 is PASS (54), PASS by substance or lineage (27), or closed by D2 (3: C-43, C-67, C-68); none is deferred and none fails. The R5 audit and hardening pass (CHANGELOG) gave every module of Tracks A–D an academic pass whose problem set carries both a proof and a computational problem, added CRA.11–17 to the cyber companion, and added the `r5_blocks` gates and the C-NEW-03, C-69 and C-73 probes. After a fresh checkout run `chmod a-w inputs-original/*`.
...
| R5 Academic depth + conflict close-out (D17) | **done** | 2026-09-24 | `refactor-tools/r5_acad.py` (rules R5-1…R5-16, run inside `r2b_build.py`) applying `authored/academic/*`; the Go katas in `authored/academic/katas/`; `kit-verification.md` (C-50, C-56, C-NEW-02); the regenerated ledger (C-23, C-66); `verification-report-R5.md`; `course/` |
| Post-R5 gap fills (learn-anything.xyz cross-check; build rule prefix R6, not the meta prompt's R6) | **done** | 2026-09-25 | `refactor-tools/r6_gaps.py` (rule R6-1, run inside `r2b_build.py`); authored additions to A6.D4, A9.D9, GOT.2, GOT.5, DBT.9, GO-11, GO-18; problems A9-P10, GOT-P9, GOT-P10, DBT-P15 with keys; CHANGELOG R6. The ledger is no longer copied to `course/`. R6-2 (same day): roadmap.sh and College Compendium cross-check; 19 gaps taught in their owner modules (rule R6-2 plus authored A4.D6, A7.D4, A7.D5, CRA.12, GO-24, GO-25, §0.6; records G22–G23); CHANGELOG R6-2 |
| R7 Course guide (D18; build rule prefix R7, not the meta prompt's phase) | **done** | 2026-09-25 | `refactor-tools/r7_guide.py` (rules R7-1…R7-6, run inside `r2b_build.py`) with `authored/guide/` → `COURSE-GUIDE.md`; gates in `audit_r2b.py` and `verify.py` read the guide; `verify.py --stage R5` PASS |
| R8 FDE companion (D19; build rule prefix FDE, run as `r8_fde.py`) | **done** | 2026-09-25 | `refactor-tools/r8_fde.py` (rules FDE-0…FDE-16) with `authored/fde-companion.md` and `authored/fde/main-course.md` → `fde-companion.md` (the seventh part), main course D5 with its academic pass D5.D and D5-P1…D5-P7, the brief's builds in A2, A3, A4, C4 and GO-21, and the brief's teaching rules merged into rules 0.2, 0.4.1, 0.4.2, 0.4.9 and 0.5; gates extended to seven parts; `verify.py --stage R5`, `audit_r2b.py` and `selfcontained.py` PASS |
| R9 The architect's practice (D20; build rule prefix PR, run as `r9_practice.py`) | **done** | 2026-09-25 | `refactor-tools/r9_practice.py` (rules PR-1…PR-6) with `authored/practice/main-course.md`; guide rule 0.4.12, the aim, the ledger's decision journal and frontier list, seven register rows (`authored/guide/`); `verify.py` plans B6 |
| R10 ML system-design case studies (D21; build rule prefix ML, run as `r10_mlcases.py`) | **done** | 2026-09-25 | `refactor-tools/r10_mlcases.py` (rules ML-1…ML-8) with `authored/mlcases/main-course.md`; a guide row, §2 IDs and seven register rows (`authored/guide/`); `verify.py` plans D6 |
| R10a Case-study decomposition, summaries and links (D22; run inside `r10_mlcases.py`) | **done** | 2026-09-25 | D6.K (K1…K28) with prerequisites and industry-pattern tables; Appendix M entries with D6.K IDs, summaries and links (294 summarised, 5 link-only); `selfcontained.py` allows https links in Appendix M only; guide rule 2 names the exception; CS 329S verified |
| R6–R10 (the meta prompt's phases) | **closed by the learner** (2026-09-24: "forget the remaining revamp prompt") | | |
...
| D17 | "after fixing those conflicts, all the course files should still line up properly and no content should be missed out. Also … include instructions to enhance, increase and add more academic material … so that this course along with the companion course files become a full practical cloud+system architecture design course with all undergraduate pre-requisites taught in academic depth and thoroughness. Use the internet for this." Then: forget the rest of the revamp prompt; put all the course files in a course file folder. | Rule 0.4.10 (academic depth) in all six parts; academic passes (D blocks) in 20 main-course modules; an academic section in each companion; problem sets with keys (149, each with an expected wrong answer); 23 Go katas for the design patterns, tested offline on Go 1.27.1; dated web checks of the volatile GCP facts. No new module IDs and nothing from `gcp-curriculum.md` (D16). The remaining conflicts are closed with probes in `verify.py`. The course is copied to `course/`. |
| D18 | "create a new .md file which stores the literal course outline, ToCs, etc which has references to the course files … a guide to the agent to teach the learner the entire course … Ensure no overlap, repeated information exists in any of the course files." (2026-09-25) | The shared rules (0.1–0.5), the one ownership register and the outline live once, in `COURSE-GUIDE.md`, the only file that names course files. Each part's §0 keeps only what is particular to it and cites rules as "rule 0.x". This replaces D6's "same contract text in every part"; D6's ban on file names still binds the six parts. |
| D19 | "ingest fde-ccdvf-course-meta-prompt.md and integrate it into the course/ directory. Not just adding it, modify it such that there should not be any repeating content and the curriculum stays unified. Update the course guide accordingly." (2026-09-25) | The brief is split by owner. Its teaching rules join the guide's rules. Its stage items that the course already taught stay with their owners, which gain only the builds and checkpoints they lacked. What the course did not teach becomes main course D5 (the parent, with its academic pass) and a seventh part, the Forward Deployed Engineer companion, whose §1 maps every brief item to its owner. Conflicts: the brief's calibrating question yields to rule 0.2's woven checks (it becomes the first of them); its module IDs A1…E7 and exam domains D1–D8 collide with main-course IDs and are not kept (the domains are CF1…CF8); its in-file progress log is the ledger delta of rule 0.4.8; its stage order yields to the main course's phase plan (rule 0.1). |
| D20 | "So the course should be such that it trains the learner to think like a bleeding edge FDE and software systems architect and become one too." (2026-09-25) | The audit found the knowledge taught but not the practice. Each missing piece goes to one owner: main course B6 The Architect's Practice (decisions and reversibility, build or buy and lock-in, evolutionary architecture and migration, teams and strategy, writing and review, learning from failure, keeping current, the road to the role; academic pass B6.D1–B6.D3 on decision analysis, real options and calibration; B6-P1…B6-P6; capstone B6.C, the course's last, which takes FDE-CAP5's system to a review board); C7's case library of public postmortems; guide rule 0.4.12 (judgment turn, decision journal, failure recall, frontier turn, design review) so every session exercises it. The aim is stated once, in the main course's "Why this order"; the guide points to it. No new part (guide §4.3). |
| D21 | "Also refer @gcp-curriculum.md file for a 309 case study material present, only copy and integrate material related to those case studies and nothing else. Ingest, analyse and understand it. Report back once finished." (2026-09-25) | The source's case-study atlas (Part 9c) and index (Appendix M) become main course D6 Production ML System Design. D6 has the method, ten families with worked cases, three builds (ranker, ETA, fraud on tokens), the academic pass D6.D1–D6.D4 and D6-P1…D6-P8. The index becomes Appendix M: 309 articles as 299 entries (a multi-part article is one entry), mapped to the families. The RAG build stays with FDE-25/FDE-CAP1. The classical zoo and the PMLE product depth are left out because D1, D3 and the PMLE part own them. |
| D22 | "Marked (verify): the catalogue's attribution … and Stanford CS 329S … - for this case study content, exempt that rule. Include actual content first, if not possible then include the links." and "decompose the case studies into their respective concepts, theory, prerequisites, implementations, etc and add to the curriculum. … This avoids overlap or repeated content." (2026-09-25) | D6 gains D6.K, the one-owner decomposition of the concepts the cases need. Each Appendix M entry carries its D6.K IDs, a summary of the article written for the course and its links (an archived copy where the original is gone or blocked); five entries with no readable copy are link-only. The no-outside-links rule is lifted for Appendix M alone (gate and guide). The catalogue is linked and CS 329S verified, so both `(verify)` marks go. |
