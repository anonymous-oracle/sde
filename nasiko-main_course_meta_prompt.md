...
## Lossless reconstruction plan (mandatory)
Because the full analysis files cannot be embedded, encode their information into a structured, lossless reconstruction plan.
The plan must be detailed enough to rebuild the entire Nasiko system in Go without missing behavior.
Do NOT paste raw file contents. Instead, convert them into normalized specifications.

Required artifacts:
- **Reconstruction Blueprint**: a step-by-step build plan from foundations to production, each step listing inputs, outputs, dependencies, and acceptance criteria.
- **Traceability Matrix**: for every original file path, map to one or more Go modules with responsibilities, key functions, and interfaces.
- **Behavioral Specs**: endpoint contracts, CLI commands, background jobs, error cases, side effects, and state transitions.
- **Data Model Specs**: collection/table schemas, indexes, TTL/retention, migrations, and invariants.
- **Algorithm Specs**: pseudocode and edge-case handling for routing, registry sync, orchestration, and search.
- **Config Matrix**: all environment variables, defaults, secret handling, and config layering rules.
- **Infra Specs**: Docker, Compose, K8s/Helm, Terraform resources, and deployment topology.
- **Ops Specs**: SLOs, alerts, dashboards, runbooks, rollback steps, and disaster recovery.
- **Test Matrix**: unit, integration, e2e, load, and chaos tests mapped to each component.

Normalization rules:
- Break each requirement into atomic statements (one behavior per bullet).
- Preserve all edge cases and error handling behaviors from the analysis.
- If details are ambiguous, include both variants and flag assumptions explicitly.
- Use stable identifiers for every spec item (e.g., API-001, JOB-012, SCHEMA-005).
