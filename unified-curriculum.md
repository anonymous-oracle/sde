...
| Modern transformer architecture and inference layer (cross-cutting; `M45`, folded DL Modules 2-6, folded IIT Modules 12-13, `ML-14`) | none confirmed | `TB-DL-006` only | `GAP:` the curriculum's transformer content stops at the 2017-2020 era. Verified absent from this file by word-boundary search: RMSNorm, rotary position embeddings, SwiGLU, grouped-query attention, mixture-of-experts as a taught mechanism, flash attention, paged attention, continuous batching, and speculative decoding. Group-relative and sequence-level policy optimization for reasoning are likewise absent, and proximal policy optimization appears once in a table rather than as taught content. Quantization, distillation, pruning and decoding strategies are already covered and are not part of this gap. `TB-DL-006` is the only known textbook-shaped candidate and is self-published and single-sourced; the primary sources remain the architecture and systems papers |
...
| `ML-8` LLM half | `TB-DL-005` Chs. 2-4, 7 | `TB-NLP-001`, `TB-DL-003`, `TB-DL-006` | tokenization, transformer blocks at architecture level, context windows and instruction tuning are now closed. `GAP:` prompts as a discipline, structured outputs, prompt injection, safety filters, and grammar-constrained decoding still have no confirmed textbook source |
