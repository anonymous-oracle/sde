...
- Batch B14 additions: Hernan & Robins, *Causal Inference: What If* (CRC, 2020) at `confirmed` level, closing the causal gap for `ML-12` and `MML-9`; Hyndman & Athanasopoulos *Forecasting* 3e, MacKay, Forsyth & Ponce 2e, Lakshmanan et al., and Tunstall et al., all `single-sourced`. See §17.1.7, §17.1.9, §17.1.11, §17.1.14 and §17.1.15.
- Batch B13 leads, not yet confirmed: Boyd & Vandenberghe, *Introduction to Applied Linear Algebra* (Cambridge, 2018), `single-sourced`; Riley, Hobson & Bence, *Mathematical Methods for Physics and Engineering* 3e, `candidate` and the most promising route to closing the `M35`/`M36` second-source gap.
...
- Added in batch B12: Raschka, *Build a Large Language Model (From Scratch)* (Manning, 2024) at `confirmed` level -- the only confirmed textbook covering `M45` attention and the folded IIT Module 9 LoRA material. Kumaresan, *Under The Hood* (Leanpub, 2026) is recorded as a `candidate` for the modern-architecture and inference gap. See §17.1.12.
- Added in batch B13: Huyen, *AI Engineering* (O'Reilly, 2025) and Shalev-Shwartz & Ben-David, *Understanding Machine Learning* (Cambridge, 2014), both `confirmed`; Alammar & Grootendorst, *Hands-On Large Language Models*, `single-sourced`; Raschka's forthcoming *Build a Reasoning Model* as a `candidate`. §17.1.16 adds twenty-six arXiv-verified primary sources for the modern transformer, serving and post-training layer.
...
| `SRC-MATH-043` | Google Books; independent preview ToC | Boyd and Vandenberghe, *Introduction to Applied Linear Algebra* (2018), chapter listing and ISBN | [Google Books edition](https://books.google.com/books?vid=ISBN9781316518960) | yes |
| `SRC-MATH-044` | Stephen Boyd/Stanford; author book site | *Introduction to Applied Linear Algebra*, edition identity, publisher and free full text; no chapter ToC exposed | [Author book site](https://web.stanford.edu/~boyd/vmls/) | yes |
| `SRC-MATH-045` | Google Books; independent preview record | Riley, Hobson and Bence, *Mathematical Methods for Physics and Engineering*, 3rd ed. (2006), edition identity and ISBN; no chapter list exposed | [Google Books edition](https://books.google.com/books?vid=ISBN9780521679718) | yes |
...
| `SRC-DL-008` | Manning Publications; publisher metadata | Raschka, *Build a Large Language Model (From Scratch)* (2024), edition identity, ISBN, year and page count; product page exposes topics rather than a chapter list | [Manning edition](https://www.manning.com/books/build-a-large-language-model-from-scratch) | yes |
| `SRC-DL-009` | O'Reilly; publisher ToC | *Build a Large Language Model (From Scratch)*, full seven-chapter and five-appendix structure, recovered by expanding the publisher's own collapsed contents listing | [O'Reilly edition](https://www.oreilly.com/library/view/build-a-large/9781633437166/) | yes |
| `SRC-DL-010` | Google Books; independent preview ToC | *Build a Large Language Model (From Scratch)*, matching chapter and appendix structure and ISBN | [Google Books edition](https://books.google.com/books?vid=ISBN9781633437166) | yes |
| `SRC-DL-011` | Leanpub; publisher-hosted scope and structure | Kumaresan, *Under The Hood* (2026), thirty-six-project structure, completion state and length; self-published, no independent record | [Leanpub edition](https://leanpub.com/under-the-hood) | yes |
| `SRC-REC-001` | fanout.sh (Suraj Gaud); independent educational recommendation | Selection signal only. Its public reading shelf corroborates `TB-DL-001`, `TB-MATH-004` and Bishop, and surfaced `TB-DL-005` and `TB-DL-006`. Solo-authored commercial platform; course content is paywalled and unverifiable; the site loads third-party advertising trackers. Never counts as a ToC source and cannot clear a `GAP:` | [Public reading shelf](https://fanout.sh/ai/resources) | yes |
| `SRC-ML-005` | Cambridge University Press; publisher ToC/metadata | Shalev-Shwartz and Ben-David, *Understanding Machine Learning* (2014), three-part twenty-three-chapter structure and ISBNs | [Cambridge edition](https://www.cambridge.org/core/books/understanding-machine-learning/3059695661405D25673058E43C8BE2A6) | yes |
| `SRC-ML-006` | Google Books; independent preview ToC | *Understanding Machine Learning*, matching part and chapter structure and ISBN | [Google Books edition](https://books.google.com/books?vid=ISBN9781107057135) | yes |
| `SRC-DL-012` | O'Reilly; publisher ToC | Huyen, *AI Engineering* (2025), full ten-chapter structure, recovered by expanding the publisher's collapsed contents listing | [O'Reilly edition](https://www.oreilly.com/library/view/ai-engineering/9781098166298/) | yes |
| `SRC-DL-013` | Google Books; independent preview record | *AI Engineering*, ISBN, year, and the same topic sequence stated in prose | [Google Books edition](https://books.google.com/books?vid=ISBN9781098166304) | yes |
| `SRC-DL-014` | O'Reilly; publisher ToC | Alammar and Grootendorst, *Hands-On Large Language Models* (2024), full twelve-chapter structure, recovered the same way | [O'Reilly edition](https://www.oreilly.com/library/view/hands-on-large-language/9781098150952/) | yes |
| `SRC-DL-015` | Google Books; independent preview record | *Hands-On Large Language Models*, ISBN, year and topical scope; no ordered chapter list exposed | [Google Books edition](https://books.google.com/books?vid=ISBN9781098150969) | yes |
| `SRC-DL-016` | Manning Publications; publisher metadata | Raschka, *Build a Reasoning Model (From Scratch)*, ISBN, stated eight-chapter and seven-appendix shape, and forthcoming June 2026 publication date | [Manning edition](https://www.manning.com/books/build-a-reasoning-model-from-scratch) | yes |
| `SRC-PAP-001` | arXiv (Cornell University); authoritative metadata API | Exact title, author list, arXiv identifier, submission year and latest version for every paper listed in §17.1.16 | [arXiv API](https://export.arxiv.org/api/query) | yes |
| `SRC-REC-002` | fanout.sh (Suraj Gaud); independent educational recommendation | Selection and topic-inventory signal only. Its public formula index at `/labs/math-decoder` publishes 382 equation-level slugs, which evidenced the two coverage gaps recorded in §17.1.12.1. Lesson bodies are paywalled and were not accessed | [Public formula index](https://fanout.sh/labs) | yes |
| `SRC-CAU-001` | Miguel Hernan; author book site | Hernan and Robins, *Causal Inference: What If*, three-part structure, publisher and free full text | [Author book site](https://miguelhernan.org/whatifbook) | yes |
| `SRC-CAU-002` | Routledge/CRC Press; publisher ToC | *Causal Inference: What If*, full three-part twenty-three-chapter structure | [Publisher edition](https://www.routledge.com/Causal-Inference-What-If/Hernan-Robins/p/book/9781420076165) | yes |
| `SRC-ML-007` | OTexts (Hyndman and Athanasopoulos); author-publisher ToC and full text | *Forecasting: Principles and Practice*, 3rd ed. (2021), thirteen-chapter structure | [Author edition](https://otexts.com/fpp3/) | yes |
| `SRC-ML-008` | Google Books; independent preview record | *Forecasting: Principles and Practice*, 3rd ed., edition identity and ISBN; no chapter list exposed | [Google Books edition](https://books.google.com/books?vid=ISBN9780987507136) | yes |
| `SRC-PROB-013` | Google Books; independent preview ToC | MacKay, *Information Theory, Inference and Learning Algorithms* (2003), part and chapter structure and ISBN | [Google Books edition](https://books.google.com/books?vid=ISBN9780521642989) | yes |
| `SRC-CV-007` | Pearson; publisher ToC (International Edition front matter) | Forsyth and Ponce, *Computer Vision: A Modern Approach*, 2nd ed., full part, chapter and section structure | [Pearson contents](https://www.pearson.de/media/muster/toc/toc_9781292014081.pdf) | yes |
| `SRC-CV-008` | Google Books; independent preview record | *Computer Vision: A Modern Approach*, 2nd ed. (2012), edition identity and ISBN; no chapter list exposed | [Google Books edition](https://books.google.com/books?vid=ISBN9780136085928) | yes |
| `SRC-MLSYS-003` | O'Reilly; publisher ToC | Lakshmanan, Robinson and Munn, *Machine Learning Design Patterns* (2020), eight-chapter structure, DOM-expanded | [O'Reilly edition](https://www.oreilly.com/library/view/machine-learning-design/9781098115777/) | yes |
| `SRC-MLSYS-004` | Google Books; independent preview ToC | *Machine Learning Design Patterns*, ISBN and the first four chapters only | [Google Books edition](https://books.google.com/books?vid=ISBN9781098115784) | yes |
| `SRC-NLP-003` | O'Reilly; publisher ToC | Tunstall, von Werra and Wolf, *Natural Language Processing with Transformers*, revised ed. (2022), eleven-chapter structure, DOM-expanded | [O'Reilly edition](https://www.oreilly.com/library/view/natural-language-processing/9781098136789/) | yes |
| `SRC-NLP-004` | Google Books; independent preview record | *Natural Language Processing with Transformers*, revised ed., edition identity and ISBN; no chapter list exposed | [Google Books edition](https://books.google.com/books?vid=ISBN9781098136796) | yes |
| `SRC-PROB-014` | Routledge/Chapman and Hall; publisher ToC | Blitzstein and Hwang, *Introduction to Probability*, 2nd ed. (2019), thirteen-chapter structure and appendices | [Publisher edition](https://www.routledge.com/Introduction-to-Probability-Second-Edition/Blitzstein-Hwang/p/book/9781138369917) | yes |
| `SRC-PROB-015` | Google Books; independent preview ToC | *Introduction to Probability*, 2nd ed., matching chapter structure and ISBN | [Google Books edition](https://books.google.com/books/about/Introduction_to_Probability_Second_Editi.html?id=1_-GDwAAQBAJ) | yes |
| `SRC-AUT-003` | Cengage Learning India; publisher ToC | Sipser, *Introduction to the Theory of Computation*, 3rd ed. (2013/2014 India release), full three-part ten-chapter structure and ISBN | [Cengage India catalog](https://www.cengage.co.in/book-list/print/introduction-to-the-theory-of-computation-66) | yes |
| `SRC-AUT-004` | Google Books; independent preview record | *Introduction to the Theory of Computation*, 3rd ed., edition identity and ISBN; no chapter list exposed | [Google Books edition](https://books.google.com/books?vid=ISBN9781133187790) | yes |
| `SRC-ML-009` | William Hamilton/McGill University; author-hosted ToC and pre-publication full text | *Graph Representation Learning* (2020), nine-chapter three-part structure | [Author book site](https://www.cs.mcgill.ca/~wlh/grl_book/) | yes |
| `SRC-ML-010` | Springer; publisher ToC/metadata | *Graph Representation Learning* (Synthesis Lectures on AI and ML), matching chapter structure and pagination | [Springer edition](https://link.springer.com/book/10.1007/978-3-031-01588-5) | yes |
| `SRC-OPT-007` | SIAM; publisher chapter listing | Beck, *First-Order Methods in Optimization* (2017), full fifteen-chapter structure | [SIAM edition](https://epubs.siam.org/doi/book/10.1137/1.9781611974997) | yes |
| `SRC-OPT-008` | Google Books; independent preview record | *First-Order Methods in Optimization*, edition identity and ISBN only; no independent chapter list exposed | [Google Books edition](https://books.google.com/books/about/First_Order_Methods_in_Optimization.html?id=xLk4DwAAQBAJ) | yes |
...
| `TB-MATH-020` | Boyd and Vandenberghe, *Introduction to Applied Linear Algebra: Vectors, Matrices, and Least Squares* (2018); applied/computational | Vectors, linear functions, norm and distance, clustering, linear independence; matrices, matrix examples and multiplication, inverses and linear equations; least squares and its data-fitting and classification applications, then multi-objective, constrained and nonlinear least squares, with a complexity appendix | `M9`, `M13`, `M26`, `MML-3`, `MATH-LA` applied tier | ToC `SRC-MATH-043`; author site `SRC-MATH-044` exposes no chapter list; `single-sourced, unverified` |
| `TB-MATH-021` | Riley, Hobson and Bence, *Mathematical Methods for Physics and Engineering*, 3rd ed. (2006); applied/engineering | Publisher scope: complex variables, integral transforms, tensors, special functions, ordinary and partial differential equations, and the mathematics of physical systems | `M35`, `M36`, `MATH-CALC-NUM` | edition metadata `SRC-MATH-045`; no chapter ToC reached from any legitimate source; `candidate` |
...
| `CORR-006` | No appraisal existed for commercial course platforms as evidence | `fanout.sh` was appraised in batch B12 and admitted as `SRC-REC-001`, an independent educational recommendation and selection signal only. It is solo-authored and commercial, its course content is paywalled and therefore unverifiable, and its public pages load third-party advertising trackers. It never counts as a ToC source and cannot clear a `GAP:`. It is not added as a curriculum resource or teaching source. Recorded so a future pass does not re-litigate it. |
...
| `TB-PROB-008` | Blitzstein and Hwang, *Introduction to Probability*, 2nd ed. (2019); applied/story-driven | Chs. 1-2: counting arguments and conditional probability including Bayes' rule; Chs. 3-6: random variables and their distributions, expectation, continuous random variables, and moment generating functions; Chs. 7-9: joint distributions, transformations, conditional expectation; Chs. 10-13: inequalities and limit theorems, Markov chains, Markov chain Monte Carlo, Poisson processes; appendices on mathematics review, R, and distribution tables | `MML-5`, `MML-6`, `PROB-STAT-INFO`, S9 | `SRC-PROB-014`, `SRC-PROB-015`; `confirmed` |
...
| `TB-PROB-007` | David J. C. MacKay, *Information Theory, Inference and Learning Algorithms* (2003); rigorous | Part I: introduction to information theory, probability, entropy and inference, data compression, the source-coding theorem, symbol and stream codes, noisy-channel coding and its theorem, dependent random variables, error-correcting codes and real channels; Part II: message passing, probabilistic inference, clustering, exact inference by enumeration, Monte Carlo methods, variational methods, independent component analysis and latent-variable models, neural networks, and sparse-graph, low-density parity-check, convolutional, turbo and fountain codes | `MML-8`, `M40`, `PROB-STAT-INFO`, `ML-CORE` | ToC `SRC-PROB-013`; `single-sourced, unverified`; the author-hosted site and Cambridge were both unreachable in this pass |
...
| `MML-5` | `TB-PROB-001`, `TB-PROB-002` Chs. 3-6, `TB-PROB-008` | `TB-PROB-005` | closed at two confirmed sources for every listed sub-topic: events, conditional probability, Bayes, independence, random variables, expectation, variance, named distributions, sampling, and convergence |
| `MML-6` | `TB-PROB-002`, `TB-PROB-003`, `TB-PROB-008` Ch. 12 | `TB-PROB-004` | closed for estimators, bias and variance, the central limit theorem, maximum likelihood, confidence intervals, hypothesis tests, p-values, and now Markov chain Monte Carlo from a second confirmed source. `GAP:` the bootstrap and jackknife rest on `TB-PROB-004` alone; A/B testing, CUPED, and multiple-testing correction have no textbook owner and are carried by the production-ML batch |
| `MML-8` | none confirmed | `TB-PROB-006`, `TB-PROB-007` | `GAP:` entropy, cross-entropy, KL divergence, mutual information, and perplexity are now carried by two independent `single-sourced, unverified` records rather than one, which is a real improvement in coverage but still clears no gap. Neither Cambridge nor the MacKay author site was reachable in this pass; retry both to promote `TB-PROB-007` |
...
| `TB-OPT-004` | Amir Beck, *First-Order Methods in Optimization* (2017); rigorous | Chs. 1-4: vector-space review, extended real-valued functions, subgradients, conjugate functions; Chs. 5-7: smoothness and strong convexity, the proximal operator, spectral functions of matrices; Chs. 8-11: primal and dual projected subgradient methods, mirror descent, the proximal gradient method, block proximal gradient; Chs. 12-15: dual-based proximal gradient, the generalized conditional gradient method, alternating minimization, ADMM | `MML-7`, `M39` optimization slice | ToC `SRC-OPT-007`; `SRC-OPT-008` gives edition identity only; `single-sourced, unverified` |
...
| `MML-7` | `TB-ENG-001` Chs. 22-23 | `TB-OPT-002`, `TB-OPT-003`, `TB-OPT-004` | `GAP:` three optimization texts are now on file, each `single-sourced, unverified`, up from two. Linear programming and the assignment problem are covered by one confirmed record; convexity, Lagrange and KKT conditions, gradient descent variants, momentum and adaptive optimizers, proximal methods, and ADMM are not covered by any confirmed record. This remains the largest open gap in the mathematics half of the audit |
...
| `TB-CV-004` | Forsyth and Ponce, *Computer Vision: A Modern Approach*, 2nd ed. (2012); rigorous/applied | Part I, image formation: geometric camera models and calibration, light and shading, colour perception and representation; Part II, early vision from one image: linear filters and convolution, sampling and aliasing, Fourier analysis, scale and image pyramids, local features including gradients, corners, SIFT and HOG, texture representation and denoising; Part III, multiple images: stereopsis and epipolar geometry, structure from motion under calibrated, affine and projective cameras; Part IV, mid-level vision: segmentation by clustering, watershed, mean shift and normalized cuts, model fitting with the Hough transform, RANSAC and expectation-maximization, and tracking with Kalman filters; later parts: range data, learning to classify, image classification, and object detection | `M42`, `ML-9`, `DL-NLP-CV-AUDIO` vision gate, S16 | ToC `SRC-CV-007`; `SRC-CV-008` gives edition identity only; `single-sourced, unverified` |
...
| `M42` | `TB-CV-002` for the geometry chapters | `TB-CV-001`, `TB-CV-003`, `TB-CV-004` | `GAP:` still no confirmed record for the core image-processing content, but it is now carried by two independent books rather than one: `TB-CV-001` Chs. 3-5 and 9, and `TB-CV-004` Part II for convolution, sampling and aliasing, Fourier analysis, pyramids, local features and denoising. Both remain `single-sourced, unverified`; morphology and histogram equalization are specific to `TB-CV-001` |
...
| `TB-NLP-002` | Tunstall, von Werra and Wolf, *Natural Language Processing with Transformers*, revised ed. (2022); applied | Chs. 1-3: the transformer landscape, text classification with pretrained models, and the anatomy of the architecture built up component by component; Chs. 4-7: multilingual named-entity recognition and cross-lingual transfer, text generation and decoding strategies, summarization, question answering; Chs. 8-11: making transformers efficient in production through distillation, quantization and pruning, working with few or no labels, training a transformer from scratch, and future directions | `M45`, `ML-8`, folded IIT Modules 7 and 12-13, `GENAI-RAG-AGENTS`, `DL-NLP-CV-AUDIO` | ToC `SRC-NLP-003`; edition identity `SRC-NLP-004`; `single-sourced, unverified` |
...
| `TB-AUT-002` | Michael Sipser, *Introduction to the Theory of Computation*, 3rd ed. (2013); rigorous | Part 1: regular languages and context-free languages; Part 2: the Church-Turing thesis, decidability, reducibility, and advanced computability topics; Part 3: time complexity, space complexity, intractability, and advanced complexity topics | `M44` discrete prerequisite, `PROOF-DISCRETE` | ToC `SRC-AUT-003`; `SRC-AUT-004` gives edition identity only; `single-sourced, unverified` |
...
| `M44` | none confirmed | `TB-AUT-001`, `TB-AUT-002` | `GAP:` the discrete-math prerequisite (finite automata, regular languages, decidability) now has two independent unverified sources rather than one. Weighted finite-state transducers themselves are still not covered by any textbook record at any evidence level: the H, C, L and G composition, determinization, minimization, epsilon removal, and disambiguation symbols remain documented only by the OpenFst and Kaldi project documentation already cited in the module |
...
| `TB-ML-003` | Shalev-Shwartz and Ben-David, *Understanding Machine Learning: From Theory to Algorithms* (2014); rigorous/theory | Part I: the formal learning model, probably-approximately-correct learnability, empirical risk minimization, uniform convergence, the bias-complexity tradeoff, VC dimension, nonuniform learnability and the runtime of learning; Part II: linear predictors, boosting, model selection and validation, convex learning, regularization and stability, stochastic gradient descent, support vector machines, kernels, multiclass and ranking problems, decision trees, nearest neighbour, neural networks; Part III: online learning, clustering, dimensionality reduction, generative models, feature selection, and advanced theory including covering numbers and compression bounds | `ML-3`, `ML-4`, `ML-CORE`, `MML-12`, `MML-7` partial, S11 | `SRC-ML-005`, `SRC-ML-006`; `confirmed` |
| `TB-ML-005` | William L. Hamilton, *Graph Representation Learning* (2020); rigorous/applied | Chs. 1-2: motivations for graph representation learning, then background in graph theory and traditional network-analysis approaches; Part I node embeddings: neighborhood-reconstruction methods including random-walk embeddings, and multi-relational data with knowledge graphs; Part II graph neural networks: the graph neural network model, practical training and design choices, and theoretical motivations and expressive power; Part III generative graph models: traditional graph-generation approaches and deep generative models of graphs | `ML-10`, folded DL Module 7 graph slice, `MML-9` graph half | `SRC-ML-009`, `SRC-ML-010`; `confirmed` |
...
| `ML-2` | none confirmed | `TB-MLSYS-002` Chs. 2-3, `TB-DL-007` Ch. 8, `TB-MLSYS-001` Ch. 5 | `GAP:` no confirmed record, but feature engineering is no longer absent: data-representation and problem-framing patterns, dataset engineering, and feature work are each carried by an unverified record. The hashing trick and train/serve parity specifically remain unlocated |
| `ML-3` | `TB-ML-002`, `TB-PROB-002`, `TB-ML-003` Chs. 11 and 4-6 | `TB-ML-001` Ch. 7 | closed for train/validation/test splits, cross-validation, model selection, confusion matrices, precision, recall, and ROC, and now for the theory beneath them -- empirical risk minimization, uniform convergence, the bias-complexity tradeoff and VC dimension. `GAP:` ranking metrics such as MAP, NDCG and MRR, and calibration, still have no confirmed source |
...
| `ML-10` | `TB-ML-005` | `TB-IR-001` link-analysis chapters | closed: graph theory foundations, random-walk and neighborhood-based node embeddings, graph neural networks, and graph generative models. `GAP:` label propagation and entity-resolution blocking specifically are not named in `TB-ML-005` and remain uncovered |
| `ML-11` | `TB-ENG-001` Chs. 22-23 optimization half; `TB-ML-002` survival and regression partial | `TB-ML-004` | the forecasting hole now has a record: `TB-ML-004` covers decomposition, exponential smoothing, autoregressive models, seasonality, hierarchical reconciliation, backtesting and prediction intervals. `GAP:` it remains `single-sourced, unverified`, so a second confirmed forecasting text is still needed |
| `ML-CORE` | `TB-ML-002`, `TB-ML-003`, `TB-IR-001`, `TB-PROB-003` | `TB-ML-001`, `TB-PROB-004` | supervised learning, evaluation, trees and ensembles are covered, and statistical learning theory -- PAC learnability, empirical risk minimization, VC dimension, regularization and stability -- is now closed by `TB-ML-003`; features, unsupervised density modelling, retrieval and ranking beyond IR, graph methods, and forecasting are not |
...
| `TB-DL-005` | Sebastian Raschka, *Build a Large Language Model (From Scratch)* (2024); applied/implementation | Chs. 1-2: what large language models are, then text data handling, byte-pair tokenization, embeddings and sampling windows; Chs. 3-4: attention mechanisms built up from simplified to causal to multi-head form, then a full GPT-style decoder assembled and used to generate text; Chs. 5-7: pretraining on unlabelled text with loss, evaluation and checkpointing plus loading pretrained weights, fine-tuning for classification, and instruction fine-tuning with preference data; appendices: a PyTorch primer, references, exercise solutions, training-loop refinements, and parameter-efficient fine-tuning with LoRA | `M45`, folded DL Modules 5-6, folded IIT Module 9, `ML-8`, `ML-14`, `DL-NLP-CV-AUDIO`, `GENAI-RAG-AGENTS` | `SRC-DL-009`, `SRC-DL-010`; edition identity `SRC-DL-008`; selection `SRC-REC-001`; `confirmed` |
| `TB-DL-007` | Chip Huyen, *AI Engineering: Building Applications with Foundation Models* (2025); implementation/operations | Chs. 1-2: what distinguishes foundation-model engineering from earlier ML work, and how these models are trained, sampled from and adapted; Chs. 3-4: evaluation methodology for open-ended output, including model-as-judge, and evaluating whole systems rather than models; Chs. 5-8: prompt engineering, retrieval augmentation and agents, fine-tuning, and dataset construction; Chs. 9-10: inference optimization covering latency and cost levers, then deployment architecture and user-feedback loops | folded IIT Modules 7-13, `M46`, `ML-3`, `ML-8`, `ML-13`, `GENAI-RAG-AGENTS`, `ML-SYS-MLOPS`, S17 | `SRC-DL-012`, `SRC-DL-013`; `confirmed`; the second source states the same sequence in prose rather than as a numbered list |
...
| `TB-DL-006` | Ramchand Kumaresan, *Under The Hood: Build Every Layer of a Large Language Model from Scratch* (2026); implementation | Thirty-six build-and-break projects: early projects cover learning mechanics, character prediction, tokenization, attention and a GPT model; middle projects cover flash attention, pretraining, distributed training, key-value caching and speculative decoding, grouped-query attention, context extension and mixture-of-experts routing; later projects cover scaling laws, fine-tuning, evaluation, reward models, RLHF, DPO, reasoning and tool use, then quantization, retrieval augmentation, multimodal and alternative architectures | `M45` modern-primitive gap, folded IIT Modules 9 and 12-13, `ML-14` | scope and structure `SRC-DL-011`; selection `SRC-REC-001`; self-published with no independent record; `candidate` |
| `TB-DL-008` | Alammar and Grootendorst, *Hands-On Large Language Models* (2024); applied | Chs. 1-3: what language models are, tokens and embeddings, and a guided tour of the internals of a running model; Chs. 4-5: text classification, then clustering and topic modelling; Chs. 6-8: prompt engineering, advanced generation techniques and tooling, semantic search and retrieval-augmented generation; Chs. 9-12: multimodal models, building text-embedding models, and fine-tuning both representation and generation models | `ML-8`, folded IIT Modules 7-8, `GENAI-RAG-AGENTS`, `M45` | ToC `SRC-DL-014`; `SRC-DL-015` gives ISBN and scope but no ordered chapter list; `single-sourced, unverified` |
| `TB-DL-009` | Sebastian Raschka, *Build a Reasoning Model (From Scratch)*; forthcoming, Manning, dated June 2026 | Stated shape is eight chapters and seven appendices, moving from inference-time reasoning techniques to modifying the model itself: reinforcement learning with verifiable rewards, group-relative policy optimization and its refinements, format rewards, and distillation from stronger reasoning models into smaller ones; appendices cover references, solutions, model source, larger models, batching, evaluation and chat interfaces | `ML-12` RL half, folded IIT Modules 7 and 9, `M45` reasoning gate | metadata `SRC-DL-016`; not yet published and no chapter titles released; `candidate` |
...
| `M45` (attention and transformers) | `TB-DL-005` Chs. 3-4 | `TB-DL-002`, `TB-DL-003`, `TB-DL-006`, `TB-NLP-001` | closed for the 2017-2020 core: scaled dot-product and causal attention, multi-head projections, decoder masking, and a GPT-style model assembled and trained end to end. Tokenization and embeddings are closed by the same record's Ch. 2. `GAP:` the modern-primitive layer below; and the BERT/T5 encoder families remain carried only by unverified records |
...
| `ML-14` | `TB-DL-001`, `TB-DL-005` | `TB-DL-002`, `TB-DL-003`, `TB-DL-006` | closed: tensors, computational graphs, multilayer perceptrons, backpropagation, regularization and optimizers from the first record; attention heads, positional encodings, residual connections and decoder masking from the second |
| Modern transformer architecture and inference layer (cross-cutting; `M45`, folded DL Modules 2-6, folded IIT Modules 9 and 12-13, `ML-14`) | none confirmed | `TB-DL-006`, `TB-DL-009` | `GAP:` the curriculum's transformer content stops at the 2017-2020 era. Verified absent from this file by word-boundary search: RMSNorm, rotary position embeddings, SwiGLU, grouped-query and multi-query attention, mixture-of-experts as a taught mechanism, flash attention, paged attention, continuous batching, and speculative decoding. Group-relative and sequence-level policy optimization for reasoning are likewise absent, and proximal policy optimization appears once in a table rather than as taught content. Quantization, distillation, pruning and decoding strategies are already covered and are not part of this gap. The textbook layer stays open; **§17.1.16 supplies verified primary sources for every item listed here**, which is the correct evidence class for material this recent |
| Distributed training and serving arithmetic (cross-cutting; folded IIT Modules 12-13, `MLSYS-3`, `MLSYS-4`, `MLSYS-8`, `M46`, `MML-10`) | none confirmed | `TB-DL-006`, `TB-DL-007` Ch. 9 | `GAP:` a second cluster, surfaced by the public formula index at `SRC-REC-002` and confirmed absent by word-boundary search: tensor, pipeline and data parallelism; ring all-reduce and collective-latency modelling; the roofline bound and arithmetic intensity; floating-point-operation accounting; micro-batch sizing; key-value cache memory arithmetic; and the arithmetic of quantization itself -- scale, zero point, group size, symmetric schemes. The file names quantization as a concept seventeen times but never its arithmetic. `TB-DL-007` Ch. 9 covers inference optimization at the decision level rather than the arithmetic level. See §17.1.16 for verified primary sources |
...
| `ML-8` LLM half | `TB-DL-005` Chs. 2-4 and 7, `TB-DL-007` Chs. 5-6 | `TB-NLP-001`, `TB-NLP-002`, `TB-DL-003`, `TB-DL-006` | tokenization, transformer blocks at architecture level, context windows, instruction tuning and prompting are now closed. `GAP:` structured outputs, prompt injection, safety filters, and grammar-constrained decoding still have no confirmed textbook source |
| Folded IIT Module 7 (prompt engineering) | `TB-DL-007` Ch. 5 | `TB-DL-008` Chs. 6-7, `TB-NLP-001` | closed at the level of prompt construction, evaluation and iteration. `GAP:` the specific reasoning scaffolds -- chain-of-thought, self-consistency, tree-of-thought, ReAct -- remain on the lecture bank in §6.9-6.10 and on their papers |
| Folded IIT Module 8 (RAG) | `TB-IR-001` substrate, `TB-DL-007` Ch. 6 | `TB-DL-008` Ch. 8, `TB-NLP-001` | closed for retrieval augmentation as a system pattern and for agents. `GAP:` the index internals -- IVF, product quantization, HNSW -- and cross-encoder reranking still have no confirmed textbook source |
| Folded IIT Module 9 (PEFT) | `TB-DL-005` appendix E | `TB-DL-006` | closed for LoRA as an adapter method applied to a working model. `GAP:` QLoRA and four-bit quantized adaptation specifically, and adapter-versus-base comparison methodology, remain on the cited papers |
| Folded IIT Modules 10-11 (multimodal and agents) | `TB-DL-007` Ch. 6 for agents | `TB-DL-008` Ch. 9, `TB-CV-003` Ch. 5 partial | agents are closed at the architecture level. `GAP:` cross-attention over text and image, vision encoders, plan-act-track loop mechanics, graph orchestration, and memory across stateless endpoints remain without a confirmed record |
| Folded IIT Modules 12-13 (serving, safety, hardware) | none confirmed | `TB-DL-006` | `GAP:` paged attention, continuous batching and speculative decoding have no confirmed record and are part of the cross-cutting modern-inference gap logged in §17.1.12.1. Quantization, pruning and distillation are covered as concepts elsewhere in this file but by no confirmed textbook here |
| `GENAI-RAG-AGENTS` | `TB-IR-001`, `TB-DL-007` | `TB-DL-008`, `TB-NLP-001`, `TB-DL-003` | substantially improved by `TB-DL-007`, which covers evaluation, prompting, retrieval, agents, fine-tuning, dataset work and inference optimization at `confirmed` level. `GAP:` index internals, reasoning scaffolds, and the modern-architecture cluster in §17.1.12.1 remain; §17.1.16 carries their primary sources |
...
| `TB-CAU-001` | Hernan and Robins, *Causal Inference: What If* (Chapman and Hall/CRC, 2020); rigorous | Part I, causal inference without models: defining a causal effect, randomized experiments, observational studies, effect modification and interaction, graphical representation of causal structure, confounding, selection bias, measurement bias, random variability; Part II, causal inference with models: why model at all, inverse-probability weighting and marginal structural models, standardization and the parametric g-formula, g-estimation of structural nested models, outcome regression and propensity scores, instrumental variables, causal survival analysis, variable selection; Part III, complex longitudinal data: time-varying treatments, treatment-confounder feedback, g-methods for time-varying treatments, target trial emulation, causal mediation | `ML-12` causal half, `MML-9`, `MLSYS-5`, `PROB-STAT-INFO` | `SRC-CAU-001`, `SRC-CAU-002`; `confirmed` |
...
| `TB-ML-004` | Hyndman and Athanasopoulos, *Forecasting: Principles and Practice*, 3rd ed. (2021); applied | Chs. 1-4: forecasting workflow, time-series graphics, decomposition into trend, seasonal and remainder components, and time-series features; Chs. 5-6: the forecaster's toolbox with benchmark methods, residual diagnostics, accuracy evaluation and prediction intervals, then judgmental forecasting; Chs. 7-10: time-series regression, exponential smoothing and state-space models, autoregressive integrated moving-average models, dynamic regression; Chs. 11-13: hierarchical and grouped reconciliation, advanced methods, and practical issues | `ML-11`, `MLSYS-1`, `ML-CORE` forecasting slice | ToC `SRC-ML-007`; edition identity `SRC-ML-008`; `single-sourced, unverified` |
...
| `ML-12` causal half | `TB-CAU-001` | `TB-PROB-004` causal-inference chapter | closed: causal graphs and confounding, selection and measurement bias, inverse-probability weighting, propensity scores, instrumental variables, and time-varying treatments. `GAP:` uplift modelling as a named technique is still not covered by any record |
| `MML-9` | `TB-RL-002` bandits and Markov decision processes, `TB-CAU-001` causal half | `TB-RL-001`, `TB-PROB-004` | closed: directed acyclic graphs, confounding, propensity scores and adjustment. `GAP:` difference-in-differences and uplift metrics remain uncovered by any confirmed record |
...
| `TB-MLSYS-002` | Lakshmanan, Robinson and Munn, *Machine Learning Design Patterns* (2020); implementation/design | Ch. 1: why recurring design patterns arise in machine-learning systems; Chs. 2-3: patterns for representing data and for framing the prediction problem; Ch. 4: model-training patterns; Chs. 5-6: patterns for resilient serving and for reproducibility; Chs. 7-8: responsible artificial intelligence, and how the patterns connect across a whole system | `M46`, `ML-1`, `ML-2`, `ML-13`, `MLSYS-2` to `MLSYS-7`, `ML-SYS-MLOPS` | ToC `SRC-MLSYS-003`; `SRC-MLSYS-004` corroborates the first four of eight chapters exactly; `single-sourced, unverified` |

##### 17.1.15.1 Production ML Module Audit and Gaps

| Modules or owner | Accepted records | Candidate or unverified records | Audit result |
|---|---|---|---|
| `M46` | `TB-DIST-001` data platform, `TB-DL-007` | `TB-MLSYS-001`, `TB-MLSYS-002` | `TB-DL-007` closes evaluation, deployment architecture, feedback loops and inference cost-latency reasoning at `confirmed` level, and `TB-MLSYS-002` adds a second independent treatment of training, serving and reproducibility patterns. The module stays anchored to the official Google PMLE exam guide and Vertex documentation for product specifics, which is correct for a certification target. `GAP:` the serving arithmetic cluster in §17.1.12.1 |
| `ML-13` | `TB-DL-007` Chs. 3-4 and 10 | `TB-MLSYS-001` responsible-ML material | evaluation methodology, model-as-judge, system-level evaluation and user-feedback loops are now closed. `GAP:` PII minimization, abuse resistance, fairness and human-in-the-loop review still have no confirmed record; `TB-SEC-001` in §17.1.4 remains the nearest anchor for the security and privacy half |
...
#### 17.1.16 Verified Primary Sources: Modern Transformer, Serving, and Post-Training Layer

The two coverage gaps recorded in §17.1.12.1 are gaps in the *textbook* layer. The
underlying material is well documented in the primary literature, and that
literature is the correct source for it: these mechanisms postdate the textbooks
this curriculum is built on, and several have no textbook treatment at all.

Every row below was verified against arXiv's own metadata API (`SRC-PAP-001`) for
exact title, author list, identifier, and year. These are papers, not textbooks;
under §17.1.1 they cannot clear a textbook `GAP:`, and the gap rows stay open.
They are recorded so the modules have citable primary sources rather than nothing.
Where a `TB-*` record covers the same ground, it is named.

| Cluster | Primary sources (author, year, arXiv id) | Feeds | Textbook cover |
|---|---|---|---|
| Normalization and activation variants | Zhang and Sennrich 2019, root-mean-square layer normalization, 1910.07467; Shazeer 2020, gated linear unit variants, 2002.05202 | `M45`, folded DL Modules 2-3, `ML-14` | none |
| Positional encoding beyond sinusoidal | Su et al. 2021, rotary position embedding, 2104.09864 | `M45`, `ML-14` | none |
| Attention-head economics | Shazeer 2019, single-write-head decoding, 1911.02150; Ainslie et al. 2023, grouped-query attention, 2305.13245 | `M45`, folded IIT Modules 12-13 | none |
| IO-aware attention kernels | Dao et al. 2022, 2205.14135; Dao 2023, second-generation parallelism and work partitioning, 2307.08691 | `M45`, folded IIT Modules 12-13, `MML-10` | none |
| Sparse conditional computation | Shazeer et al. 2017, sparsely-gated mixture-of-experts, 1701.06538; Fedus et al. 2021, switch routing at trillion scale, 2101.03961; Jiang et al. 2024, an open sparse-mixture model, 2401.04088 | `M45`, folded DL Module 7, folded IIT Modules 12-13 | none |
| Serving memory and scheduling | Kwon et al. 2023, paged memory management for serving, 2309.06180 | folded IIT Modules 12-13, `MLSYS-4`, `MLSYS-8`, `M46` | none |
| Speculative execution in decoding | Leviathan et al. 2022, 2211.17192; Chen et al. 2023, speculative sampling, 2302.01318 | folded IIT Modules 12-13, `MLSYS-4` | none |
| Distributed training partitioning | Shoeybi et al. 2019, intra-layer model parallelism, 1909.08053; Huang et al. 2018, pipeline parallelism with micro-batching, 1811.06965; Rajbhandari et al. 2019, optimizer and gradient state sharding, 1910.02054 | folded IIT Modules 12-13, `MLSYS-8`, `M46` | none |
| Post-training quantization arithmetic | Dettmers et al. 2022, mixed-precision integer matrix multiplication, 2208.07339; Frantar et al. 2022, one-shot post-training quantization, 2210.17323; Lin et al. 2023, activation-aware weight quantization, 2306.00978; Dettmers et al. 2023, quantized fine-tuning, 2305.14314 | folded IIT Modules 9 and 12-13, `MLSYS-8` | `TB-DL-005` appendix E covers the adapter half only |
| Scaling and compute allocation | Kaplan et al. 2020, 2001.08361; Hoffmann et al. 2022, compute-optimal training, 2203.15556 | `M45`, `M46`, `MLSYS-3` | none |
| Policy optimization for reasoning | Schulman et al. 2017, proximal policy optimization, 1707.06347; Shao et al. 2024, group-relative policy optimization, 2402.03300; DeepSeek-AI 2025, reasoning via reinforcement learning, 2501.12948; Zheng et al. 2025, group sequence policy optimization, 2507.18071 | `M45` post-training, `ML-12`, folded IIT Module 7 | `TB-DL-009` is forthcoming and unpublished |

Two mechanisms in the gap list have no arXiv primary source and are recorded as
such: **continuous batching** originates in the Orca serving paper (USENIX OSDI
2022), and the **roofline performance model** originates in Williams, Waterman
...
| Blitzstein and Hwang, *Introduction to Probability*, 2nd ed. (2019) — `TB-PROB-008` | Chs. 1-2 counting and conditional probability with Bayes; Chs. 3-6 random variables, expectation, continuous distributions, moments; Chs. 7-9 joint distributions, transformations, conditional expectation; Chs. 10-13 inequalities and limit theorems, Markov chains, MCMC, Poisson processes | `confirmed` |
<!-- /RESEARCH-ADDITION -->
...
| Beck, *First-Order Methods in Optimization* (2017) — `TB-OPT-004` | Chs. 1-4 vector spaces, extended real-valued functions, subgradients, conjugate functions; Chs. 5-7 smoothness, strong convexity, the proximal operator, spectral functions; Chs. 8-11 subgradient methods, mirror descent, proximal gradient, block proximal gradient; Chs. 12-15 dual-based proximal gradient, generalized conditional gradient, alternating minimization, ADMM | `single-sourced, unverified` |
...
| MacKay, *Information Theory, Inference and Learning Algorithms* (2003) — `TB-PROB-007` | Part I information theory, probability and inference, compression, source and noisy-channel coding theorems, symbol and stream codes, error-correcting codes; Part II message passing, inference, clustering, Monte Carlo and variational methods, independent component analysis, neural networks, sparse-graph and modern codes | `single-sourced, unverified` |

`GAP:` `MML-8` still needs a *confirmed* information-theory text. MacKay was subsequently located and recorded as `TB-PROB-007` from an independent preview ToC, so the module now rests on two independent unverified records rather than one. Neither the MacKay author site nor Cambridge was reachable during the research passes; retry either to promote the record.
<!-- /RESEARCH-ADDITION -->
...
| Raschka, *Build a Large Language Model (From Scratch)* (Manning, 2024) — `TB-DL-005` | Chs. 1-2 what LLMs are, text data, byte-pair tokenization, embeddings, sampling windows; Chs. 3-4 attention from simplified to causal to multi-head, then a GPT-style decoder assembled and generating text; Chs. 5-7 pretraining, loading pretrained weights, classification fine-tuning, instruction fine-tuning; appendices PyTorch primer, references, solutions, training-loop refinements, LoRA | `confirmed` |
| Kumaresan, *Under The Hood* (Leanpub, 2026) — `TB-DL-006` | Thirty-six build-and-break projects: tokenization, attention and a GPT model; flash attention, pretraining, distributed training, key-value caching, speculative decoding, grouped-query attention, context extension, mixture-of-experts; scaling laws, fine-tuning, reward models, RLHF, DPO, reasoning, tool use; quantization, retrieval augmentation, multimodal and alternative architectures | `candidate`; self-published, no independent record |
| Shalev-Shwartz and Ben-David, *Understanding Machine Learning* (Cambridge, 2014) — `TB-ML-003` | Part I formal learning model, PAC learnability, empirical risk minimization, uniform convergence, bias-complexity tradeoff, VC dimension, nonuniform learnability, runtime of learning; Part II linear predictors, boosting, model selection, convex learning, regularization and stability, SGD, SVMs, kernels, multiclass, trees, nearest neighbour, neural networks; Part III online learning, clustering, dimensionality reduction, generative models, feature selection, advanced theory | `confirmed` |
| Huyen, *AI Engineering* (O'Reilly, 2025) — `TB-DL-007` | Chs. 1-2 foundation-model engineering and how these models are trained and adapted; Chs. 3-4 evaluation methodology including model-as-judge, and system-level evaluation; Chs. 5-8 prompt engineering, retrieval augmentation and agents, fine-tuning, dataset engineering; Chs. 9-10 inference optimization for latency and cost, deployment architecture and feedback loops | `confirmed` |
| Alammar and Grootendorst, *Hands-On Large Language Models* (O'Reilly, 2024) — `TB-DL-008` | Chs. 1-3 language models, tokens and embeddings, model internals; Chs. 4-5 classification, clustering and topic modelling; Chs. 6-8 prompt engineering, advanced generation, semantic search and RAG; Chs. 9-12 multimodal models, building embedding models, fine-tuning representation and generation models | `single-sourced, unverified` |
| Raschka, *Build a Reasoning Model (From Scratch)* (Manning, forthcoming June 2026) — `TB-DL-009` | Stated as eight chapters and seven appendices: inference-time reasoning, then reinforcement learning with verifiable rewards, group-relative policy optimization and refinements, format rewards, and distillation into smaller models | `candidate`; unpublished |
| Hernan and Robins, *Causal Inference: What If* (Chapman and Hall/CRC, 2020) — `TB-CAU-001` | Part I definitions of causal effect, randomized and observational studies, effect modification, interaction, causal graphs, confounding, selection and measurement bias; Part II modelling, inverse-probability weighting, the parametric g-formula, g-estimation, propensity scores, instrumental variables, causal survival analysis; Part III time-varying treatments, treatment-confounder feedback, g-methods, target trial emulation, causal mediation | `confirmed` |
| Hyndman and Athanasopoulos, *Forecasting: Principles and Practice*, 3rd ed. (2021) — `TB-ML-004` | Chs. 1-4 workflow, graphics, decomposition, time-series features; Chs. 5-6 the forecaster's toolbox, accuracy evaluation, prediction intervals, judgmental forecasting; Chs. 7-10 regression, exponential smoothing, ARIMA, dynamic regression; Chs. 11-13 hierarchical reconciliation, advanced methods, practical issues | `single-sourced, unverified` |
| Hamilton, *Graph Representation Learning* (2020) — `TB-ML-005` | Chs. 1-2 motivations, graph theory background; Part I neighborhood-reconstruction node embeddings, multi-relational knowledge graphs; Part II the GNN model, practical training, theoretical motivations; Part III traditional and deep generative graph models | `confirmed` |
| Tunstall, von Werra and Wolf, *NLP with Transformers*, revised ed. (2022) — `TB-NLP-002` | Chs. 1-3 the transformer landscape, classification, architecture anatomy; Chs. 4-7 multilingual NER, generation and decoding, summarization, question answering; Chs. 8-11 production efficiency via distillation, quantization and pruning, few- and zero-label settings, training from scratch | `single-sourced, unverified` |

`M45` now has one confirmed textbook, `TB-DL-005`, for its 2017-2020 core. `GAP:` the modern-primitive layer -- RMSNorm, rotary position embeddings, SwiGLU, grouped-query attention, mixture-of-experts, flash attention, paged attention, continuous batching, speculative decoding, and group-relative policy optimization for reasoning -- is absent from this file entirely; see §17.1.12.1. The `GENAI-RAG-AGENTS` node beyond retrieval still has no confirmed textbook. Bishop's *Pattern Recognition and Machine Learning*, Murphy, Duda/Hart/Stork, Russell and Norvig, and Hernan and Robins were not edition-verified in this pass, so their chapter maps elsewhere in this file carry no verification claim.
<!-- /RESEARCH-ADDITION -->
...
| Forsyth and Ponce, *Computer Vision: A Modern Approach*, 2nd ed. (2012) — `TB-CV-004` | Part I camera models, calibration, light and shading, colour; Part II linear filters and convolution, sampling and aliasing, Fourier analysis, pyramids, local features including SIFT and HOG, texture, denoising; Part III stereopsis, epipolar geometry, structure from motion; Part IV segmentation by clustering and normalized cuts, model fitting with Hough, RANSAC and EM, tracking with Kalman filters; later parts range data, classification and detection | `single-sourced, unverified` |
<!-- /RESEARCH-ADDITION -->
...
| Lakshmanan, Robinson and Munn, *Machine Learning Design Patterns* (O'Reilly, 2020) — `TB-MLSYS-002` | Ch. 1 why recurring patterns arise; Chs. 2-3 data-representation and problem-representation patterns; Ch. 4 model-training patterns; Chs. 5-6 resilient serving and reproducibility; Chs. 7-8 responsible AI and how the patterns connect | `single-sourced, unverified` |

The already-`confirmed` records in §17.1.4 -- Kleppmann `TB-DIST-001`, the two Google SRE volumes, and Anderson `TB-SEC-001` -- remain the systems, operations and security substrate for `MLSYS-2`, `MLSYS-4`, `MLSYS-6`, `MLSYS-7` and `MLSYS-8`. Huyen's *AI Engineering* (`TB-DL-007`, §17.1.12) is `confirmed` and covers evaluation, deployment and inference optimization. `GAP:` the ML-specific production-pattern layer is now carried by two independent unverified records, `TB-MLSYS-001` and `TB-MLSYS-002`, but neither is confirmed.
<!-- /RESEARCH-ADDITION -->
...
<!-- RESEARCH-ADDITION §17.1 -->
§17.1.16 adds twenty-six further primary sources covering the modern transformer, serving and post-training layer, each verified against arXiv's metadata API: RMSNorm and gated-linear-unit variants, rotary position embeddings, multi-query and grouped-query attention, flash attention, sparse mixture-of-experts routing, paged memory management for serving, speculative decoding, tensor and pipeline parallelism and optimizer-state sharding, post-training quantization schemes, compute-optimal scaling, and the policy-optimization family from PPO through GRPO and GSPO. These are the correct evidence class for material that postdates the textbooks this curriculum is built on.
<!-- /RESEARCH-ADDITION -->
...
<!-- RESEARCH-ADDITION §17.1 -->
Verified text: `TB-DL-005` Chs. 2-4 (§17.1.12) closes tokenization, embeddings, scaled dot-product and causal attention, multi-head projections, decoder masking, and a GPT-style model built and trained end to end. `GAP:` the modern-primitive layer -- RMSNorm, rotary position embeddings, SwiGLU, grouped-query attention, mixture-of-experts, and flash attention -- is absent from this module and from the file; see the cross-cutting gap row in §17.1.12.1. The BERT and T5 encoder families remain carried only by `single-sourced, unverified` records.
<!-- /RESEARCH-ADDITION -->
...
<!-- RESEARCH-ADDITION §17.1 -->
Verified texts: `TB-IR-001` for the retrieval substrate and `TB-DL-007` Ch. 6 for retrieval augmentation and agents as system patterns (§17.1.13). `GAP:` IVF, product quantization, HNSW and cross-encoder reranking still have no confirmed textbook.
<!-- /RESEARCH-ADDITION -->
...
<!-- RESEARCH-ADDITION §17.1 -->
Verified text: `TB-DL-005` appendix E (§17.1.12) covers LoRA as an adapter method applied to a working model. `GAP:` QLoRA's four-bit quantized adaptation specifically, and adapter-versus-base comparison methodology, still rest on their papers.
<!-- /RESEARCH-ADDITION -->
...
<!-- RESEARCH-ADDITION §17.1 -->
`GAP:` no confirmed textbook (§17.1.13, §17.1.15). Paged attention, continuous batching and speculative decoding are absent from this file entirely and form part of the cross-cutting modern-inference gap in §17.1.12.1; `TB-DL-006` is the only known candidate and is self-published and single-sourced. Quantization, pruning and distillation are covered as concepts here but by no confirmed textbook.
<!-- /RESEARCH-ADDITION -->
